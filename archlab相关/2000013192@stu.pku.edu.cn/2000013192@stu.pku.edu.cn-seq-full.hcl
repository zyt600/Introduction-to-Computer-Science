...
#周雨童
#2000013192
#iaddq  首先需要fetch，不难发现指令是10字节，因此valp=pc+10，需要rb不需要ra，常数需要valc；其次decode，取R[rb]；其次执行，将valc、valb加起来，设置条件码；不需要访存；写回时写进rb；更新pc为valp
#icode:ifun <- M1[PC]
#rA:rB <- M1[PC+1]
#D <- M8[ PC+2 ]
#valP <- PC+10
...
#jm首先 首先需要fetch，不难发现指令是10字节，因此valp=pc+10，需要rb不需要ra，常数需要valc；其次decode，取R[rb]；其次执行，将valc、valb加起来，得到vale；访存M[vale],得到valM；更新pc时valM即新pc
#icode:ifun <- M1[PC]
#rA:rB <- M1[PC+1]
#D <- M8[ PC+2 ]
#valP <- PC+10
